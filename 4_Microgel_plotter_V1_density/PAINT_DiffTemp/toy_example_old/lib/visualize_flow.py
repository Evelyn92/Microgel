...
def plt_flow_samples(prior_sample, n_samples, transform, ax, npts=100, memory=100, title="$x ~ q(x)$", device="cpu"):
    z = prior_sample(n_samples, 2).type(torch.float32).to(device)
    zk = []
    ...
    ax.scatter(zk[:,0], zk[:,1], s=0.1, c='r', marker="o")
    ax.set_xlim(LOW, HIGH)
    ax.set_ylim(LOW, HIGH)
...
def plt_samples(samples, ax, npts=100, title="$x ~ p(x)$"):
    ax.scatter(samples[:, 0], samples[:, 1], s=0.1, c='b', marker="o")
    ax.set_xlim(LOW, HIGH)
    ax.set_ylim(LOW, HIGH)
...
def visualize_transform(
    ...
    if transform is not None:
        n_samples = potential_or_samples.shape[0]
        plt_flow_samples(prior_sample, n_samples, transform, ax, npts=npts, memory=memory, device=device)
