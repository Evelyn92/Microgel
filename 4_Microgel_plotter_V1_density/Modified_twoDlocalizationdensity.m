...
deltay=[];
UpperSolva = 400;
for micro_index = 1:20
    add = strcat("./PAINT_DiffTemp/Core-shell/",temperature,"C/Microgel_plotter_v2_solvatochromism/",string(micro_index),".3d");
...
%add = "./results/results_mtemp/gg5.ply";
    ...
    all_localiz_tmp  = dlmread(add);
    all_localiz_tmp = all_localiz_tmp(all_localiz_tmp(:,4)<UpperSolva,:);
    all_localiz = [all_localiz;all_localiz_tmp];
    zvec=[zvec;all_localiz_tmp(:,3)-median(all_localiz_tmp(:,3))];
    intens=[intens;all_localiz_tmp(:,4)];
    xcoord=all_localiz_tmp(:,1);
    deltax =[deltax; xcoord-median(xcoord)];
    ycoord=all_localiz_tmp(:,2);
    deltay = [deltay; ycoord-median(ycoord)];
...
fname = strcat("./results/visualization/",temperature,"/",temperature,'C_',string(micro_index),'.png');
saveas(gcf,fname)
...
%     countbin = count;
    ...
    %medint(medint>0) = medint(medint>0)./max(max(medint)).*0.4+0.4;
    medint(medint>0) = medint(medint>0)./max(max(medint));
    %%
