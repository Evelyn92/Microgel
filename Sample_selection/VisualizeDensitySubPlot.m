function density_vector = VisualizeDensitySubPlot(ax, density_vector, save_add_path, selected_path, index, plot_config)
        Visible = true;
...
%          pcshow(cloud);
%          
%          fname_pc = strcat(processed_path,className,'/',className,'-',string(index),className,'-pc.png');
%          saveas(g,fname_pc)
...
%         if Visible
%             ax=figure("Visible", 'on');
%         else
%             ax=figure("Visible", 'off');
%         end
        %set(ax,'Position',[20 20 figure_height_in_pixel max(h_range)/max(d_range)*figure_height_in_pixel]);
    ...
    %     countbin = count;
        ...
        set(ax,'XTick',d_range/d_dist*labelx_dist/d_dist+0.5);
        set(ax,'XTickLabel',0:labelx_dist:max(d_range));
        set(ax,'YTick',h_range/h_dist*labely_dist/h_dist+0.5);
        set(ax,'YTickLabel',-max(h_range)/2:labely_dist:max(h_range)/2);
